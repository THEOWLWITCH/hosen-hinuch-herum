...
function allowed(name: string, type: string) {
  ...
  const okExt = new Set([
    "pdf","doc","docx","ppt","pptx","xls","xlsx","csv","odt","ods","odp",
    "txt","rtf","jpg","jpeg","png","webp","gif","mp4","mov"
  ]);
...
export default async (req: Request) => {
  ...
  if (!allowed(f.name, f.type)) {
    return json({ error: "file_type", message: `סוג הקובץ ${f.name} אינו נתמך.` }, 415);
  }

  if (f.size > 4_000_000) {
    return json({
      error: "file_too_large",
      message: "העלאה ישירה מוגבלת לכ־4MB לקובץ. עבור וידאו או קובץ גדול יש לצרף קישור בתוך ההודעה."
    }, 413);
  }

  const key = `files/${id()}-${safeName(f.name)}`;
  await store("hosen-files").set(key, f, {
    metadata: { contentType: f.type || "application/octet-stream", filename: f.name }
  });
